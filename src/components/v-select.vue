<template>
 <div class="v-select">
  ...
  <div class="options" v-if="areOptionsVisible">
   <!-- при клике будем передавать значение(option) 
   из массива в элемент списка-->
   <span 
...
<script>
 export default {
  ...
  methods: {
   ...
   // т.к. никто не работает со значением option 
   // внутри компонента select момент выбирания опций
   // придется отлавливать в родиетеле,
   ...
   // через метод select-up передадим значение option
    selectOptions(option) {
      ...
      // после передачи значения выпадающий 
      // список значений закрывается
